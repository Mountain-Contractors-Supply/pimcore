...
# Certain secrets cause problems with Symfony and Pimcore when used as files - this script converts them to env vars, and then containers can source this script at startup.
...
export APP_SECRET=$(cat /run/secrets/kernel-secret 2>/dev/null || echo "")